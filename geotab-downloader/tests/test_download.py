...
def test_fetch_diagnostics_by_ids_single_diagnostic():
    ...
    mock_api.get.return_value = [
        {"id": "diag1", "name": "Diagnostic 1", "diagnosticType": "Type1", "source": "Source1"}
    ]
...
def test_fetch_diagnostics_by_ids_multiple_diagnostics():
    mock_api = MagicMock()
    diagnostics_data = {
        "diag1": {"id": "diag1", "name": "Diagnostic 1", "diagnosticType": "Type1", "source": "Source1"},
        "diag2": {"id": "diag2", "name": "Diagnostic 2", "diagnosticType": "Type2", "source": "Source2"},
    }
...
def test_download_all_downloads_log_records(tmp_path):
    ...
    log_records = [
        {"id": "log1", "dateTime": "2026-01-01", "speed": "60", "latitude": "40.7", "longitude": "-74.0"}
    ]
...
def test_download_all_no_diagnostics_when_no_status_data(tmp_path):
    mock_api = MagicMock()
    devices = [{"id": "dev1", "name": "Device 1", "deviceType": "Type1"}]
    mock_api.get.side_effect = lambda entity_type, **kwargs: devices if entity_type == "Device" else []
