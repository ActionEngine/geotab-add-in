...
@patch("check_runner.has_recent_locations")
def test_main_no_scripts_found(mock_has_recent, tmp_path, caplog):
    """Test that main exits with error when contexts references non-existent check."""
    caplog.set_level(logging.ERROR)
    mock_has_recent.return_value = True
...
@patch("check_runner.has_recent_locations")
def test_main_successful_execution(mock_has_recent, tmp_path, caplog):
...
def test_main_with_failing_check(mock_run_all_checks, mock_has_recent, tmp_path, caplog):
    caplog.set_level(logging.ERROR)
    setup_check_dirs(tmp_path)
    mock_has_recent.return_value = True
